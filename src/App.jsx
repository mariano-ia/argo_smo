...
export default function App() {
  const [phase, setPhase] = useState('idle') // idle | generating-content | generating-images | done | error
  ...
  const addToHistory = (ig, li) => {
    ...
    const updated = [...history, ...newPosts]
    ...
    <div style={styles.root}>
      ...
      <main style={styles.main}>
        ...
        <div style={styles.hero}>
          ...
          <button style={{...styles.generateBtn, opacity: isLoading ? 0.7 : 1}} onClick={generate} disabled={isLoading}>
            {isLoading ? (<><span style={styles.spinner} /> {phase === 'generating-content' ? 'Generando contenido...' : 'Generando imágenes...'}</>) : (phase === 'done' ? 'Generar de nuevo' : 'Generar contenido de hoy')}
          </button>
          {history.length > 0 && phase === 'idle' && (<div style={styles.historyHint}>Claude va a analizar tus últimos {Math.min(history.length, 12)} posts para decidir qué publicar hoy.</div>)}
        </div>
...
const styles = {
  ...
  errorBox: { background: '#FFF0F0', border: '1px solid #FFCCCC', borderRadius: 10, padding: '12px 16px', fontSize: 13, color: '#C0392B', maxWidth: 500 },
  generateBtn: { display: 'inline-flex', alignItems: 'center', gap: 10, fontSize: 16, fontWeight: 600, color: '#FFFFFF', background: '#955FB5', border: 'none', borderRadius: 14, padding: '14px 32px', cursor: 'pointer' },
